...
--功能点
INSERT INTO   pcs_prc_function   (  id  ,   name  ,   code  ,   parent_function_id  ,   sort  ,   type  ) VALUES ('03e4b44977b', '删除空间', 'workspaceDelete', NULL, 31, '2');
...
--日志--
INSERT INTO   pcs_op_log_template   (  id  ,   title  ,   content  ,   link  ,   bgroup  ,   abstract_content  ) VALUES ('LOG_CODE', '日志通用模板', '<div><span style="color: #4581b7;">${user}</span> <span>${actionType} </span><span>${mode}： </span><img src="${images}" alt="" width="16px" height="16px"/><span style="margin: 0 5px;font-weight: 600;   color: #4b768a;">${name}</span></div>', '/workspacePage', 'postin','管理员创建了默认空间');
...
--消息--
INSERT INTO   pcs_mec_message_template   (  id  ,   msg_type_id  ,   msg_send_type_id  ,   title  ,   content  ,   link  ,   bgroup  ) VALUES ('7c8fb5b0', 'WS_MS_TYPE', 'email', '邮箱空间创建模板', '<div style="  display: flex;   align-items: center;"> <span style=\"\n    color: #6c8ca0;\n    margin: 0 5px 0 0;\n\">${userName}</span>创建了:<img src=\"${images}\" alt=\"\" width=\"16px\" height=\"16px\" style=\"\n    margin: 0 5px;\"/><span  style=\"font-weight:600;overflow: hidden; text-overflow: ellipsis; white-space: nowrap;\">${name}</span></div>', NULL, 'postin');
INSERT INTO   pcs_mec_message_template   (  id  ,   msg_type_id  ,   msg_send_type_id  ,   title  ,   content  ,   link  ,   bgroup  ) VALUES ('WS_CREATE', 'WS_MS_TYPE', 'site', '空间创建通知', '<div style="   display: flex;   align-items: center;"> <span style=\"\n    color: #6c8ca0;\n    margin: 0 5px 0 0;\n\">${userName}</span>创建了:<img src=\"${images}\" alt=\"\" width=\"16px\" height=\"16px\" style=\"\n    margin: 0 5px;\"/><span  style=\"font-weight:600;overflow: hidden; text-overflow: ellipsis; white-space: nowrap;width: 120px;\">${name}</span></div>', '/workspacePage', 'postin');
