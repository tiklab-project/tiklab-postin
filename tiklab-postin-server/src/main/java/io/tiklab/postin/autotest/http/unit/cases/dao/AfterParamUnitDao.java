...
public class AfterParamUnitDao {
    ...
    */
    ...
    public void updateSortAfterDelete(String apiUnitId, int deletedSort) {
        try {
            String sql = "UPDATE autotest_action_after SET sort = sort - 1 WHERE api_unit_id = ? AND sort > ?";
            jpaTemplate.getJdbcTemplate().update(sql, apiUnitId, deletedSort);
        } catch (Exception e) {
            logger.error("更新后置操作单元排序失败", e);
        }
    }
    ...
    public void updateSortAfterDrag(String apiUnitId, int oldSort, int newSort) {
        try {
            if (oldSort < newSort) {
                ...
                String sql = "UPDATE autotest_action_after SET sort = sort - 1 WHERE api_unit_id = ? AND sort > ? AND sort <= ?";
