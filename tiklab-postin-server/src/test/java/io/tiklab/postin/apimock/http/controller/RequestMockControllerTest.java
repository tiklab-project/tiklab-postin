...
public class RequestMockControllerTest {

    private static Logger logger = LoggerFactory.getLogger(RequestMockControllerTest.class);
    ...
    public void test01ForSaveRequestMock() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(requestMock);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/RequestMock/createRequestMock")
    ...
    public void test02ForUpdateRequestMock(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/RequestMock/updateRequestMock")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindRequestMock() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/RequestMock/findRequestMock")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllRequestMock() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/RequestMock/findAllRequestMock")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteRequestMock(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/RequestMock/deleteRequestMock")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
