package com.tiklab.postlink.sysmgr.apistatus.controller;
...
import com.tiklab.postlink.sysmgr.apistatus.model.ApiStatus;
...
public class ApiStatusControllerTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(ApiStatusControllerTest.class);
    ...
    public void test01ForSaveApiStatus() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(apiStatus);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/apiStatus/createApiStatus")
    ...
    public void test02ForUpdateApiStatus(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/apiStatus/updateApiStatus")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test04ForFindAllApiStatus() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/apiStatus/findAllApiStatus")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteApiStatus(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/apiStatus/deleteApiStatus")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
