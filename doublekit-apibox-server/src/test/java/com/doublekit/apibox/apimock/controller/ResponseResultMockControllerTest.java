...
public class ResponseResultMockControllerTest {
    ...
    public void test01ForSaveResponseResultMock() {
        ...
        Map paramMap  = JSONObject.parseObject(JSONObject.toJSONString(responseResultMock));
        MultiValueMap<String, String> multiValueMap = new LinkedMultiValueMap<>();
        multiValueMap.setAll(paramMap);
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/responseResultMock/createResponseResultMock")
    ...
    public void test02ForUpdateResponseResultMock(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/responseResultMock/updateResponseResultMock")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindResponseResultMock() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/responseResultMock/findResponseResultMock")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllResponseResultMock() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/responseResultMock/findAllResponseResultMock")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteResponseResultMock(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/responseResultMock/deleteResponseResultMock")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
