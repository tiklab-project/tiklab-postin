...
import io.thoughtware.postin.support.imexport.utils.Md5;
import io.thoughtware.postin.api.apix.model.JsonParam;
import io.thoughtware.postin.api.http.definition.model.JsonResponse;
import io.thoughtware.postin.support.imexport.model.FormDataImportVo;
import io.thoughtware.postin.support.imexport.model.JsonParamImportVo;
import io.thoughtware.postin.support.imexport.model.JsonResponseImportVo;
import io.thoughtware.postin.support.imexport.model.ApixImportVo;
...
public class ReportImport {
    ...
    private void jsonParamLoop(JSONArray param,String methodId,String parentId){
        for(int i = 0;i<param.size();i++){
            ...
            //是否有model， 有 递归
            if(jsonItem.containsKey("io.thoughtware.postin.test.model")){
                jsonParamLoop(jsonItem.getJSONArray("io.thoughtware.postin.test.model"),methodId,pid);
            }
        }
    }
    ...
    private void analysisResponseJson(JSONObject methodItem, String methodId){
        ...
        JSONArray model = result.getJSONArray("io.thoughtware.postin.test.model");
        String parentId = null;
    ...
    private void jsonResponseloop(JSONArray param,String methodId,String parentId){
        for(int i = 0;i<param.size();i++){
            ...
            //是否有model， 有 递归
            if(jsonItem.containsKey("io.thoughtware.postin.test.model")){
                jsonResponseloop(jsonItem.getJSONArray("io.thoughtware.postin.test.model"),methodId,pid);
            }
        }
    }
